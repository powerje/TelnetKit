...
public enum Command: Byte, RawRepresentable {
...
public enum Option: Byte, RawRepresentable, CaseIterable {
...
//    case exopl = 255            // http://tools.ietf.org/html/rfc861
    ...
    /// Initialize all options and set their status to false.
    var options: [Option: Bool] {
        return Option.allCases.reduce(into: [:]) { $0[$1] = false }
    }
...
public extension Array where Element == Byte {
    ...
    var bytes: [Byte] { return self }

    var isTelnetCommand: Bool {
        if self.count < 2 { return false }

        let first = self[0]
        let second = self[1]
        return first == Command.iac.rawValue && second != Command.iac.rawValue
    }

    var telnetCommandList: String {
        var output = ""
        for byte in self {
            if let value = Command(rawValue: byte) {
                output.append("\(value)(\(value.rawValue)) ".uppercased())
                continue
            }
            if let value = Option(rawValue: byte) {
                output.append("\(value)(\(value.rawValue)) ".uppercased())
                continue
            }
            output.append("RAW(\(byte)) ")
        }
        return output
    }
